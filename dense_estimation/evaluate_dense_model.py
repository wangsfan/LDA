import sys; sys.path.append('/home/yjy/EventTransformerPlus-main/EventTransformerPlus-main')
        ...
        if evaluation_mode in ['performance', 'efficiency']:
            ...
            if evaluation_mode == 'performance':
                # Calculate stats
                    ...
                    output = pred[0,0,:,:,0].cpu().numpy()
                    # reg_factor = 3.70378
                    # img = np.exp(reg_factor * (output - np.ones((output.shape[0], output.shape[1]), dtype=np.float32)))
                    ...
                    # img[:,:,0:3] = img[:,:,0:3][...,::-1]
                    cv2.imwrite("fileimage.jpg", output)
