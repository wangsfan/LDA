...
import sys
sys.path.append('/home/yjy/EventTransformerPlus-main/EventTransformerPlus-main')
    ...
    def __init__(self,
        ...
        for seq in self.side:
            for i, f in enumerate(sorted(os.listdir(f'{self.path_dataset}/{self.sequence}/{seq}/depth/'))): 
                self.ts_depths[seq].append(float(f.split('_')[1].replace('.pckl','')))
        ...
        if use_images:
            ...
            for s in self.side:
                for f in sorted(os.listdir(f'{self.path_dataset}/{self.sequence}/{s}/image_raw/')): 
                    self.ts_images[s].append(float(f.split('_')[1].replace('.pckl','')))
        ...
        if use_events:
            ...
            for s in self.side:
                for f in sorted(os.listdir(f'{self.path_dataset}/{self.sequence}/{s}/{self.event_folder}/')): 
                    self.ts_events[s].append(float(f.split('_')[1].replace('.pckl','')))
...
class MVSEC_DataModule(LightningDataModule):
    def __init__(self, dataset_name, data_params, batch_size, patch_size, min_activations_per_patch, 
        ...
        if self.dataset_name == 'MVSEC':
            self.min_dist = [2]
            self.max_dist = [80]
            self.path_dataset = [f'/home/yjy/datasets/MVSEC_final/0601_dataset_frames_k{data_params["k"]}_MT{data_params["MT"]//1000}_mT{50//data_params["k"]}_v0_clear']
